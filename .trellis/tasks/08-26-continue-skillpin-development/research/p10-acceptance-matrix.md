...
The first remote P10 CI run on August 26, 2026 reproduced two fresh-checkout quality defects. Ubuntu and macOS failed `npm run lint` before type-checking because `eslint-plugin-import` resolved CLI imports of `@skillpin/core/*` through the core package exports, but a fresh checkout had no `packages/core/dist/` output yet. The root `npm run lint` command now builds `@skillpin/core` before invoking ESLint, making the documented lint entrypoint self-contained. Windows failed `npm run format:check` because checkout line-ending conversion had no repository policy; `.gitattributes` now normalizes detected text to LF on all platforms. The corrected rerun then passed Windows formatting/lint/type-check/tests/build but exposed a second Windows-only fault: `scripts/build-package.mjs` used `execFileSync("npm.cmd", ...)`, which Node cannot spawn directly on Windows. It now invokes the npm CLI through `process.execPath` and `npm_execpath` without a shell. The same corrected run also showed every browser E2E case failing before the app loaded because Vite could not resolve the core root export in the clean checkout. `npm run test:e2e` now bootstraps `@skillpin/core` before launching Playwright. All corrections must be rerun through the full three-OS CI matrix.
