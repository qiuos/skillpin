...
- **Chrome**: no sidebar or KPI cards. `.identity-bar` contains product name, `nav[aria-label="SkillPin 功能分区"]` when an authoritative source list is nonempty, connection state, and end-session action. Navigation tabs use 24px text with a 52px minimum target; on narrow screens the navigation wraps below the product name. Do not add an appearance entry or settings drawer without an explicit product requirement. Skills uses a two-window catalog/detail workbench plus an always-visible bottom command bar; detail is read-only.
