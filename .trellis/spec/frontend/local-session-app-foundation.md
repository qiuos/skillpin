...
- Theme preference may use localStorage under `skillpin.theme`; it stores only `"system" | "light" | "dark"`. When set to `system`, subscribe to `matchMedia("(prefers-color-scheme: dark)")` changes. Resolved theme is applied as `document.documentElement.dataset.theme` (`light` | `dark`) so CSS tokens in `styles.css` switch. Visual tokens are near black/white product chrome (see frontend quality-guidelines Styling); no session data belongs in localStorage.
