...
- Theme preference may use localStorage under `skillpin.theme`; it stores only `"system" | "light" | "dark"`. When set to `system`, subscribe to `matchMedia("(prefers-color-scheme: dark)")` changes. No session data belongs in localStorage.
