...
- Skills-workbench typography is fixed and scoped: a skill name/title uses `28px`; all other workbench text, including filters, status, detail prose, empty states, and action buttons, uses `24px`. Skill action controls are at least `56px` tall; rows must retain enough space for two wrapped title and summary lines plus compact source context without overlapping the action. List rows surface the primary source identity and, when applicable, the candidate count and parse-warning marker; keep redundant action-direction copy out of the read-only detail pane. Keep action controls, source chips, long row text, and candidate/Markdown content wrap-safe; use independently scrolling catalog/detail panes rather than clipping or overlaying large text. Do not restore the temporary `文字调试（临时）` panel unless a future requirement explicitly reopens typography calibration.
