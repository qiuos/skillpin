...
- Workbench density is scoped by surface: Skill Detail retains a `28px` title and `24px` prose, while the catalog uses a `22px` skill name and `22px` one-line summary. The primary search, filter trigger, filter headings, and source/status controls also use the `22px` catalog text scale with a `54px` search/filter trigger target and `48px` popover-option targets; row-level enable/remove actions remain compact at `16px` / `44px`. A catalog row is a single compact line with a fixed name-priority column, ellipsized summary, optional compact parse-warning marker, and non-overlapping explicit action. Do not show source identity, candidate count, or redundant enabled/disabled copy in a catalog row; retain source and candidate comparison context in Skill Detail. Keep popover controls, action controls, long row text, and candidate/Markdown content wrap-safe; use independently scrolling catalog/detail panes rather than clipping or overlaying large text. Do not restore the temporary `文字调试（临时）` panel unless a future requirement explicitly reopens typography calibration.
...
| Compact catalog density | The catalog toolbar stays one row with 22px primary source/filter typography, a 54px search/filter trigger, and 48px popover controls; rows use a 22px name, 22px one-line ellipsized summary, 16px/44px action, and no source/candidate/status copy. Skill Detail retains its 28px/24px reading scale. |
...
- Playwright must cover a populated root-started skills workbench with the Skills tab selected, group/candidate detail rendering, search, compact filter-popover keyboard/outside-click behavior and active-filter count, source-hidden one-line catalog rows, fresh full-height two-window rendering, 22px/22px catalog rows plus 22px primary source/filter typography with a 54px search target and 44px actions, retained 28px/24px detail typography, removed debugger controls, and long-row action separation.
