...
P9 begins only when a user clicks an explicit catalog-row, inline group-member, or group `启用` / `移除` action. Browsing catalog items, choosing a candidate, expanding a group, and reading Markdown remain read-only P8 behavior.
...
- A skill or inline group-member `启用` action uses the selected candidate when that group is currently inspected; otherwise it uses the group's first stable candidate. `移除` sends `{ linkName, candidateId: null }` only when the project snapshot marks that link as `managed`.
- A group action exposes only fixed `全部启用` and `移除` controls. It immediately derives selections for every disabled member to make the entire group enabled without replacing already-managed links, or for every managed member to remove them. It sends all derived selections in one plan/apply transaction; do not retain a browser-side staging queue.
...
- During any action, disable all catalog, inline-member, and group buttons to prevent concurrent plans or applies. The active action may show its `启用中…` or `移除中…` state.
...
| Apply success | Update project state from the returned snapshot; list and inline-member action/status copy changes immediately. |

## 5. Good / Base / Bad Cases

- **Good:** a user clicks group-level `全部启用`; the UI plans and applies every currently disabled member in one request pair, then renders `2 / 2 已启用`.
- **Base:** a user removes one expanded group member; exactly that one selection is planned/applied and the group count becomes `1 / 2 已启用`.
- **Bad:** treating a row/group expansion click as a mutation, persisting a staged selection queue, or allowing two concurrent actions.
...
- Cover direct enable/removal plus group `全部启用` and inline-member removal. Assert one plan/apply pair per explicit operation, group count refresh, fixed group controls, no `启用剩余` copy, and no batch command bar/confirmation dialog.
