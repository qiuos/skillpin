...
- `packages/core/src/platform/index.ts`, `packages/core/src/changes/index.ts`, and `packages/core/src/persistence/index.ts` expose Node-only P1/P2 APIs without pulling `node:fs` or `node:crypto` into the Web bundle.
...
- Do not export Node-only modules from `@skillpin/core` package root, because Web imports that root. Use the `@skillpin/core/platform`, `@skillpin/core/changes`, or `@skillpin/core/persistence` subpath instead.
