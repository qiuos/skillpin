...
- `SourceRuntime.projectSelections()` initializes the current session catalog and resolves each candidate ID from that live snapshot. It builds the Node-only P4 `ProjectSelection` with canonical target data. A missing ID returns `CATALOG_CANDIDATE_NOT_FOUND` with `review-state` recovery.
...
| Selection candidate no longer exists in session catalog | `422 CATALOG_CANDIDATE_NOT_FOUND`, `review-state` |
...
- **Base:** the browser repeats the same successful `requestId`; P4 returns the cached result with `idempotent: true` and makes no second filesystem change.
...
- Integration tests start a P5 session, prove unauthenticated P9 access is rejected, add a real source, obtain a candidate ID from catalog data, plan/apply through P4, assert real linked target ownership, repeat for idempotency, and verify stale/missing-candidate failures.
